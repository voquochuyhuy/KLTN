...
class EventManagement extends Component {
  ...
  componentDidMount() {
    axios
      .get(`http://localhost:3000/graphql?query=${getEvents}`)
      .then((result) => {
        ...
        let parsingdata = events.map((event, key) => {
          let startTimeParsed = new Date(event.startTime).toLocaleString(
            "vi-VN"
          );
      ...
      db.collection("questions").orderBy('createdAt', 'desc').limit(5).get().then(docs => {
        docs.forEach(doc => {
  ...
  render() {
    ...
    return (
      <GridContainer>
        <GridContainer>
          <GridItem xs={10}>
            {/* <Button color="success" onClick={this.openDialog}>
              Thêm sự kiện
            </Button> */}
          </GridItem>
          <GridItem xs={2}>
            <Button color="success" onClick={this.logout}>
              Đăng xuất
            </Button>
          </GridItem>
        </GridContainer>
